...
CATEGORY_EMOJIS = {
    ...
    "Results": "📊", "Discussion": "💬", "Conclusion": "🏁", "Writing": "✍️",
    "Research": "🔎", "Data Analysis": "📈", "Meetings": "👥", "Other": "🔧"
}
...
def load_data():
    if os.path.exists(DATA_FILE):
        try:
            ...
            data = {'progress': {}, 'reports': [], 'categories': [], 'tasks': [], 'todo': [], 'tags': []}
    else:
        data = {'progress': {}, 'reports': [], 'categories': [], 'tasks': [], 'todo': [], 'tags': []}
    return data
...
# Sidebar navigation
pages = ["Home 🏠", "To-Do List 📋", "Tasks and Reports 📝", "Progress 📊", "Statistics 📈", "Manage Categories 🏷️", "Gantt Chart 📅", "Export/Import 💾"]
...
async def run_timer(placeholder, duration, is_break=False):
    ...
    while datetime.now() < end_time and st.session_state.timer_running:
        ...
        placeholder.markdown(f'<p class="time">{mins:02d}:{secs:02d}</p>', unsafe_allow_html=True)
...
for task in st.session_state.data['todo']:
    st.sidebar.write(f"{PRIORITY_EMOJIS[task['priority']]} {task['name']} (Due: {task['due_date']})")
...
if st.session_state.page == "Home 🏠":
    ...
    with st.form("new_todo"):
        ...
        category = st.selectbox("Category", [f"{CATEGORY_EMOJIS[cat]} {cat}" for cat in st.session_state.data['categories']])
        category = category.split(" ", 1)[1]
        priority = st.selectbox("Priority", list(PRIORITY_EMOJIS.keys()))
        due_date = st.date_input("Due Date")
        estimated_time = st.number_input("Estimated Time (hours)", min_value=0.0, step=0.5)
        steps = st.text_area("Steps (one per line)")
        tags = st.multiselect("Tags", options=st.session_state.data['tags'] + ["Add new tag..."])
        ...
        if submitted and task_name:
            new_task = {
                ...
                "steps": [{"step": step.strip(), "completed": False} for step in steps.split('\n') if step.strip()],
                "tags": tags,
                "completed": False,
                "actual_time": 0,
                "notes": ""
    ...
    filter_category = st.multiselect("Filter by Category", options=st.session_state.data['categories'])
    filter_priority = st.multiselect("Filter by Priority", options=list(PRIORITY_EMOJIS.keys()))
    filter_tags = st.multiselect("Filter by Tags", options=st.session_state.data['tags'])
    sort_by = st.selectbox("Sort by", options=["Due Date", "Priority", "Estimated Time"])
    ...
    for i, task in enumerate(filtered_tasks):
        ...
        with col1:
            completed = st.checkbox("Complete", value=task['completed'], key=f"todo_{i}")
        with col2:
            ...
            with expander:
                ...
                for j, step in enumerate(task['steps']):
                    step_completed = st.checkbox(step['step'], step['completed'], key=f"step_{i}_{j}")
                    if step_completed != step['completed']:
                        st.session_state.data['todo'][st.session_state.data['todo'].index(task)]['steps'][j]['completed'] = step_completed
                        save_data(st.session_state.data)
                st.write(f"Tags: {', '.join(task['tags'])}")
                notes = st.text_area("Notes", task['notes'], key=f"notes_{i}")
                if notes != task['notes']:
                    st.session_state.data['todo'][st.session_state.data['todo'].index(task)]['notes'] = notes
                    save_data(st.session_state.data)
        with col3:
        ...
        if completed != task['completed']:
            st.session_state.data['todo'][st.session_state.data['todo'].index(task)]['completed'] = completed
            if completed:
    ...
    with st.form("new_report"):
        ...
        task = st.selectbox("Task", ["New task..."] + st.session_state.data['tasks'] + [t['name'] for t in st.session_state.data['todo']])
        if task == "New task...":
    ...
    reports = sorted(st.session_state.data['reports'], key=lambda x: x['date'] if isinstance(x['date'], datetime) else datetime.fromisoformat(x['date']), reverse=True)
    
    # Group reports by week
    week_reports = {}
    for report in reports:
        report_date = report['date'] if isinstance(report['date'], datetime) else datetime.fromisoformat(report['date'])
    ...
    for week_start, week_data in week_reports.items():
        with st.expander(f"Week of {week_start} 📅"):
            for i, report in enumerate(week_data):
                ...
                with col1:
                    st.write(f"**{report_date.strftime('%Y-%m-%d %H:%M')}** - {CATEGORY_EMOJIS[report['category']]} {report['category']}: {report['task']}")
    ...
    fig = go.Figure(data=[go.Pie(labels=[f"{CATEGORY_EMOJIS[s]} {s}" for s in sections], values=values, hole=.3, marker_colors=colors)])
    ...
    for section, color in zip(sections, colors):
        ...
        progress = st.slider(f"{section} Progress", 0, 100, 
                             st.session_state.data['progress'].get(section, 0), key=section)
    ...
    if reports:
        ...
        df_time = pd.DataFrame([(datetime.fromisoformat(r['date']).date() if isinstance(r['date'], str) else r['date'].date(), r['time_spent']) for r in reports], columns=['Date', 'Time Spent'])
        df_time = df_time.groupby('Date').sum().reset_index()
        st.subheader("Time Spent Over Time ⏱️")
        st.line_chart(df_time.set_index('Date'))
        
        # Average ratings over time
        df_ratings = pd.DataFrame([(datetime.fromisoformat(r['date']).date() if isinstance(r['date'], str) else r['date'].date(), r['result_rating'], r['focus_rating']) for r in reports], 
        ...
        df_category = pd.DataFrame([(r['category'], r['time_spent']) for r in reports], columns=['Category', 'Time Spent'])
        df_category = df_category.groupby('Category').sum().reset_index()
        st.subheader("Time Spent by Category")
        fig = px.pie(df_category, values='Time Spent', names='Category', title='Time Distribution Across Categories')
        st.plotly_chart(fig)
        
        # Productivity score over time (combination of result and focus ratings)
        df_productivity = pd.DataFrame([(datetime.fromisoformat(r['date']).date() if isinstance(r['date'], str) else r['date'].date(), (r['result_rating'] + r['focus_rating'])/2) for r in reports], 
    ...
    # Remove category
    category_to_remove = st.selectbox("Select Category to Remove", st.session_state.data['categories'])
    ...
    if tasks:
        df = pd.DataFrame([
            dict(Task=task['name'], Start=task['due_date'] - timedelta(days=int(task['estimated_time']/8)), 
                 Finish=task['due_date'], Resource=task['category'])
            for task in tasks
        ])
    ...
    if st.button("Export Data"):
        ...
        st.success("Data exported successfully! Click the 'Download JSON' button to save the file.")
